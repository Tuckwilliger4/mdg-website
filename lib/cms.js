...
// Build-time cache to prevent duplicate API calls (server-side only)
let buildCache
if (typeof window === 'undefined') {
  buildCache = new Map()
}

// Hygraph GraphQL client with caching
async function hygraphRequest(query, variables = {}) {
  // Only use caching on server-side during build
  const cacheKey = typeof window === 'undefined' ? JSON.stringify({ query, variables }) : null
  
  // Check cache first (server-side only)
  if (cacheKey && buildCache && buildCache.has(cacheKey)) {
  ...
  if (cacheKey && buildCache) {
    buildCache.set(cacheKey, data)
    console.log('Cached new data for query')
  }
